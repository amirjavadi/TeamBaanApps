...
export default class AboutSuperVisor extends React.Component {
  ...
  componentDidMount() {
    axios.get(api.url + '/api/Content/2')
      .then((response) => {
        let about = response.data.more.replace(/[&\/\\#,+()$~%'":*?<>{}]/g, '');
        let newAbout = about.replace(/[A-Za-z]/g, '');
        let lastAbout = newAbout.replace(/[;]/g, ' ');
        this.setState({about: lastAbout})
      })
  }
  ...
  render() {
    return(
      <Container>
        <Header name="درباره پشتیبان"/>
        <View style={{position: 'absolute', width: deviceWidth * 2, height: deviceHeight / 4, backgroundColor: '#5dd243', marginLeft: -deviceWidth / 2, marginTop: -deviceHeight / 10, transform: [{rotate: '-12deg'}], elevation: 5, zIndex: 10}}>
        ...
        <Content>
          ...
          {this.state.about !== '' && <View style={{width: deviceWidth - 40, marginHorizontal: 20, direction: 'rtl', paddingTop: deviceHeight / 10 + deviceHeight / 20, flexDirection: 'column', alignItems: 'flex-start', justifyContent: 'flex-start'}}>
            <View style={{flexDirection: 'row', marginBottom: 10}}>
              <Icon type="FontAwesome" name="tags" style={{color: '#5dd243', fontSize: 20, marginHorizontal: 5}}/>
              <Text style={{...Platform.select({
                ...
                }), color: '#333333', fontSize: 14}}>درباره پشتیبان</Text>
            </View>
