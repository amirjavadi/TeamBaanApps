...
import {Container, Text, View, Icon, Thumbnail} from 'native-base';
import {Dimensions, Image, Platform, FlatList, TouchableOpacity, Animated} from 'react-native';
...
export default class WorkShops extends React.Component {
  ...
  getContents() {
    axios.get(api.url + '/api/Categories/7/Contents/?page=' + this.state.pageNumber + '&pagesize=10&$expand=Files')
      .then((response) => {
        let data = [];
        if (response.data.length < 1) {
          let obj = {
            title: 'امروز کارگاهی موجود نیست.',
            summery: '',
          };
          data.push(obj);
  ...
  goToDetails(item) {
    if (Actions.currentScene === 'workShopDetails'){
    } else {
      Actions.workShopDetails({item: item})
    }
  }
  ...
  render() {
    ...
    return(
      <Container>
        <Header name="کارگاه ها"/>
        <View style={{position: 'absolute', width: deviceWidth * 2, height: deviceHeight / 4, backgroundColor: '#e66bae', marginLeft: -deviceWidth / 2, marginTop: -deviceHeight / 10, transform: [{rotate: '-12deg'}], elevation: 5, zIndex: 10}}>
        ...
        <View style={{width: deviceWidth - 40, marginHorizontal: 20, direction: 'rtl', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start'}}>
          <Animated.View style={{width: deviceWidth, position: 'absolute', flexDirection: 'row', marginTop, alignItems: 'flex-start', paddingHorizontal: 20}}>
            <Icon type="FontAwesome" name="bookmark" style={{color: '#e66bae', fontSize: 20, marginHorizontal: 5}}/>
            <Text style={{...Platform.select({
              ...
              }), color: '#333333', fontSize: 14}}>کارگاه ها</Text>
          </Animated.View>
