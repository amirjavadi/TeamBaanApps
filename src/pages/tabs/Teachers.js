...
import {Container, View, Text, Icon, Thumbnail} from 'native-base';
...
export default class Teachers extends React.Component {
  ...
  getContents() {
    axios.get(api.url + '/api/Categories/8/Contents/?page=' + this.state.pageNumber + '&pagesize=10&$expand=Files')
      .then((response) => {
        let data = [];
  ...
  goToDetails(item) {
    if (Actions.currentScene === 'teacherDetails'){
    } else {
      Actions.teacherDetails({item: item})
    }
  }
  ...
  render() {
    ...
    return(
      <Container>
        <Header name="مشاوره و مربیان"/>
        <View style={{position: 'absolute', width: deviceWidth * 2, height: deviceHeight / 4, backgroundColor: '#2bd674', marginLeft: -deviceWidth / 2, marginTop: -deviceHeight / 10, transform: [{rotate: '-12deg'}], elevation: 5, zIndex: 10}}>
        ...
        <View style={{width: deviceWidth - 40, marginHorizontal: 20, direction: 'rtl', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start'}}>
          <Animated.View style={{width: deviceWidth, position: 'absolute', flexDirection: 'row', marginTop, alignItems: 'flex-start', paddingHorizontal: 20}}>
            <Icon type="FontAwesome" name="bookmark" style={{color: '#2bd674', fontSize: 20, marginHorizontal: 5}}/>
            <Text style={{...Platform.select({
              ...
              }), color: '#333333', fontSize: 14}}>مشاوره و مربیان</Text>
          </Animated.View>
